...
# ==================== XianyuSliderStealth 终极绕过补丁 ====================
try:
    from utils.xianyu_slider_stealth import XianyuSliderStealth
    # 由于我们已经在二进制文件中将原始的 _check_date_validity 重命名为 _old_check_date_vldt，
    # 现在的 _check_date_validity 调用会指向我们在这里定义的 Mock 函数。
    def mock_check_date_validity(self):
        # logger.success("【补丁】已成功拦截并绕过滑块组件日期检查")
        return True
    
    # 注入 Mock 函数
    XianyuSliderStealth._check_date_validity = mock_check_date_validity
    logger.success("【补丁】已成功应用 XianyuSliderStealth 终极重命名绕过补丁")
except Exception as patch_e:
    logger.warning(f"【补丁】应用 XianyuSliderStealth 补丁失败: {patch_e}")
# =========================================================================
